...
static void inline test_create_principal() {
  ...
  printf("attest of 10.10.0.3:%d --- %s\n", 20000, att_client.attest("10.10.0.3", 20000, "").c_str());
  printf("attest of 10.10.0.3:%d --- %s\n", 1001, att_client.attest("10.10.0.3", 1001, "").c_str());
  printf("attest of 10.10.0.3:%d --- %s\n", 1090, att_client.attest("10.10.0.3", 1090, "").c_str());
}
...
static void inline test_endorse_image() {
  ...
  latte::MetadataServiceClient att_client(safe_url, ATTEST_ID);
  printf("attest of 10.10.0.3:%d --- %d\n", 20000, att_client.has_property("10.10.0.3", 20000, "git://docker", ""));
  printf("attest of 10.10.0.3:%d --- %d\n", 1001, att_client.has_property("10.10.0.3", 1001, "git://pio", ""));
  printf("attest of 10.10.0.3:%d --- %d\n", 1090, att_client.has_property("10.10.0.3", 1090, "git://spark", "")); }
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      printf("attesting 10.10.0.3:%d, result: %d\n", ports[i],
          att_client.can_access("10.10.0.3", ports[i], obj(objs[j]), ""));
    }
  }
}
